...
/**
 * The ServerUDP class listens for UDP packets from clients, processes them, and sends responses.
 * It continuously listens on a specified port, and upon receiving a message, it sends a response back to the client.
 */
public class ServerUDP {
   ...
   */
  public ServerUDP(int port, double requestReceiveProbability, double replySendProbability, ServerNetwork serverNetwork) throws SocketException {
  ...
  /**
   * Receives a UDP packet, processes the message, and decides whether to continue running.
   * If the message is successfully processed, a response is sent back to the client.
   *
  ...
  private boolean receiveAndSend() throws IOException {
    ...
    DatagramPacket packet = new DatagramPacket(buf, buf.length); // Create a packet to store the incoming data
    socket.receive(packet); // Receive the incoming data and store it in the packet

    byte[] responseData = serverNetwork.processRequest(packet.getData()); // Process the incoming data and get a response
  ...
  /**
   * Sends a UDP packet to the specified client address and port.
   *
   ...
   * @throws IOException if an I/O error occurs.
   */
  private void send(byte[] responseData, InetAddress clientAddress, int clientPort) throws IOException {
    if (!toSend()) {
      System.out.println("Packet loss from server to client.");
      return;
    }

    DatagramPacket responsePacket = new DatagramPacket(responseData, responseData.length, clientAddress, clientPort); // Create a packet to store the response data
    socket.send(responsePacket); // Send the response data to the client
  }

  /**
   * Stops the server from listening for further messages and releases any system resources associated with it.
   */
  public void close() {
